...
namespace CashbackAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        {
            return Ok(new Revendedor());
        }

        [HttpPost("register")]
        [AllowAnonymous]
        public async Task<IActionResult> Register(Revendedor revendedor)
        {
            try
            {
                var user = _mapper.Map<User>(revendedor);

                var result = await _userManager.CreateAsync(user, revendedor.Senha);

                var userToReturn = _mapper.Map<Revendedor>(user);
